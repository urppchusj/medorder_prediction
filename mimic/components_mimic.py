...
class data:
	...
	def __init__(self, subset='traintest'):
		# Prepare the data paths given a directory.
		# File names correspond to what is created by the preprocessor.
		if subset == 'traintest':
			profiles_filename='profiles_list.pkl'
			targets_filename='targets_list.pkl'
			seq_filename='seq_list.pkl'
		...
		self.activemeds_file = os.path.join(os.getcwd(), 'mimic', 'preprocessed_data_mimic', activemeds_filename)
		self.depa_file = os.path.join(os.getcwd(), 'mimic', 'preprocessed_data_mimic', depa_filename)
		self.enc_file = os.path.join(os.getcwd(), 'mimic', 'preprocessed_data_mimic', enc_filename)
	...
	def make_lists(self, get_valid=True):
		...
		shuffled = list(zip(self.targets_train, self.seq_train, self.active_meds_train, self.depa_train))
		random.shuffle(shuffled)
		self.targets_train, self.seq_train, self.active_meds_train, self.depa_train = zip(*shuffled)
...
class TransformedGenerator(keras.utils.Sequence):
	...
	def __init__(self, w2v, pse, le, y, X_w2v, X_am, X_depa, w2v_embedding_dim, sequence_length, batch_size, shuffle=True, return_targets=True):
	...
	def __getitem__(self, idx):
		...
		batch_pse = [[bm, bd] for bm, bd in zip(batch_am, batch_depa)]
		# Transform
	...
	def on_epoch_end(self):
		...
		if self.shuffle == True:
			shuffled = list(zip(self.y, self.X_w2v, self.X_am, self.X_depa))
			random.shuffle(shuffled)
			self.y, self.X_w2v, self.X_am, self.X_depa = zip(*shuffled)
